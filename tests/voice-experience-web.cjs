...
async function boot(browser,width=1440,height=900,mobile=false){
 ...
 await page.addInitScript(()=>{
  ...
  const makeStream=()=>{
   const C=window.AudioContext||window.webkitAudioContext,ctx=new C(),dst=ctx.createMediaStreamDestination(),osc=ctx.createOscillator(),gain=ctx.createGain();
   gain.gain.value=.00001;osc.connect(gain).connect(dst);osc.start();window.__ycTestMic={ctx,osc,stream:dst.stream};return dst.stream;
  };
  ...
  try{Object.defineProperty(navigator,'mediaDevices',{configurable:true,value:media})}catch{try{navigator.mediaDevices.getUserMedia=media.getUserMedia;navigator.mediaDevices.enumerateDevices=media.enumerateDevices}catch{}}
 });
 const supabase=mock();
 ...
 try{
  for(const [width,height,mobile] of [[1440,900,false],[390,844,true],[844,390,true]]){
   ...
   const rules=await page.evaluate(()=>({
    online:window.YamachatVoiceExperience.messageSoundAllowedFor('online'),
    dnd:window.YamachatVoiceExperience.messageSoundAllowedFor('dnd'),
    invisible:window.YamachatVoiceExperience.messageSoundAllowedFor('invisible'),
    join:window.YamachatVoiceExperience.announcementText('join','Petr'),
    leave:window.YamachatVoiceExperience.announcementText('leave','Petr'),
    speaking:window.YamachatVoiceExperience.presenceFor('online',true,true,60*60*1000,5,'afk'),
    silent:window.YamachatVoiceExperience.presenceFor('online',true,false,6*60*1000,5,'online'),
    dndPresence:window.YamachatVoiceExperience.presenceFor('dnd',true,false,60*60*1000,5,'online'),
    invisiblePresence:window.YamachatVoiceExperience.presenceFor('invisible',true,false,60*60*1000,5,'online'),
    outsideFallback:window.YamachatVoiceExperience.presenceFor('online',false,false,0,5,'afk'),
    snapshot:window.YamachatVoiceExperience.snapshot()
   }));
   assert.equal(rules.online,true);assert.equal(rules.dnd,false);assert.equal(rules.invisible,false);
   assert.equal(rules.join,'Petr se připojil do místnosti');assert.equal(rules.leave,'Petr opustil místnost');
   assert.deepEqual({speaking:rules.speaking,silent:rules.silent,dnd:rules.dndPresence,invisible:rules.invisiblePresence,outside:rules.outsideFallback},{speaking:'online',silent:'afk',dnd:'dnd',invisible:'invisible',outside:'afk'});
   ...
   await page.waitForSelector('#soundboardPanel:not(.hidden)');
   ...
   assert.equal(after.voiceConnected,true,JSON.stringify({before,after}));
   assert.equal(after.voiceChannelId,before.voiceChannelId,JSON.stringify({before,after}));
   assert.equal(after.voiceSessionId,before.voiceSessionId,JSON.stringify({before,after}));
   assert.equal(after.voiceCommunityId,'community-a',JSON.stringify({before,after}));
   assert.equal(after.micLive,true,JSON.stringify({before,after}));
   assert.equal(after.heartbeat,true,JSON.stringify({before,after}));

   assert.deepEqual(errors,[]);
   await page.close();
  }
  console.log('PASS voice experience: themed bars, settings, DND/invisible sound rules, named announcements, soundboard controls, voice-aware AFK and real UI cross-server voice persistence.');
