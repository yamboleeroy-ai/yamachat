(()=>{
 ...
 function replace(panel,button){
  if(!mobile.matches||!panel||!button)return false;
  // Navigation actions may change panel CSS outside this controller. Always
  // normalize the old drawer state before opening the requested destination.
  close();
  returnFocus=button;activeDrawer=panel;panel.inert=false;
  panel.classList.add(panel.id==='side'?'mobile-open':'yc-mobile-open');
  document.getElementById('app')?.classList.add('yc-mobile-drawer-open');
  button.setAttribute('aria-expanded','true');
  panel.querySelector('button,a,input,[tabindex="0"]')?.focus();
  return true;
 }
 function openFriendsDrawer(){
  const right=document.querySelector('.yc-v3-content-grid>.right');
  const button=document.getElementById('ycMobileMembersBtn');
 ...
 // DM buttons stop propagation in the social list, so handle DM/thread taps in
 // capture phase and close the active mobile drawer after the click dispatch.
 // Profile clicks nested inside a DM row intentionally keep their own modal flow.
 document.addEventListener('click',e=>{
  if(!mobile.matches||e.target.closest?.('[data-profile-open]'))return;
  if(e.target.closest?.('[data-thread],[data-dm]'))queueMicrotask(close);
 },true);
 ...
 document.addEventListener('click',e=>{
  if(!mobile.matches)return;
  if(!e.target.closest?.('#ycV3Friends,#ycOpenFriendsList'))return;
  queueMicrotask(()=>openFriendsDrawer());
 });
