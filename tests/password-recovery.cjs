...
const recoverySource=fs.readFileSync(path.join(root,'web/password-recovery.js'),'utf8');
for(const marker of ["fragment.get('token_hash')","fragment.get('access_token')","fragment.get('refresh_token')","client.auth.setSession(credentials)","client.auth.verifyOtp({token_hash:token,type:'recovery'})"]) assert(recoverySource.includes(marker),'recovery source missing dual-flow marker '+marker);

(async()=>{
 const browser=await chromium.launch({headless:true,...(process.env.BROWSER_EXECUTABLE?{executablePath:process.env.BROWSER_EXECUTABLE}:{})});
 try{
  // Existing custom token_hash flow and recovery-request form.
  for(const scenario of ['request','unknown','rate-limit','offline','reset','expired','weak','mismatch','invalid-type']){
   ...
   await page.route('**/*',async route=>{
    ...
    if(u.hostname==='bxjvmjdppmqgbxfcowpf.supabase.co'){
     ...
     if(req.method()==='OPTIONS')return route.fulfill({status:204,headers:{'access-control-allow-origin':'*','access-control-allow-headers':'*','access-control-allow-methods':'POST,PUT,GET,OPTIONS'}});
     ...
     if(u.pathname.endsWith('/user')&&req.method()==='PUT'){
      updates++;
      if(scenario==='weak'&&updates===1){status=422;response={code:'weak_password',msg:'Weak password'}}
      else response={id:'11111111-1111-4111-8111-111111111111',email:'test@example.com',aud:'authenticated'};
     }
     if(u.pathname.endsWith('/user')&&req.method()==='GET')response={id:'11111111-1111-4111-8111-111111111111',email:'test@example.com',aud:'authenticated'};
   ...
   assert.equal(calls.length,0,'Opening a token_hash email must not consume its token');
   ...
   if(request){
   ...
   }else if(scenario==='invalid-type'){
    assert(await page.locator('#passwordForm').isHidden());assert(await page.locator('#retryLink').isVisible());
   }else{
    await page.locator('#newPassword').fill('ExamplePassword123!');
    await page.locator('#confirmNewPassword').fill(scenario==='mismatch'?'DifferentPassword123!':'ExamplePassword123!');
    ...
    if(scenario==='mismatch'){
      assert.equal(calls.length,0);assert.match(await page.locator('#recoveryStatus').innerText(),/neshodují/);
    }else if(scenario==='expired'){
      assert.equal(calls.filter(c=>c.path.endsWith('/user')&&c.method==='PUT').length,0);assert(await page.locator('#retryLink').isVisible());
    }else{
      if(scenario==='weak'){
        assert.match(await page.locator('#recoveryStatus').innerText(),/nesplňuje/);
        await page.locator('#passwordSubmit').click();
        await page.waitForFunction(()=>!document.getElementById('passwordSubmit').disabled);
      }
      assert.equal(calls.filter(c=>c.path.endsWith('/verify')).length,1);
      assert.equal(calls.find(c=>c.path.endsWith('/verify')).body.type,'recovery');
      assert.match(await page.locator('#recoveryStatus').innerText(),/Heslo bylo změněno/);
      assert(await page.locator('#passwordForm').isHidden());
    }
   }
  ...
  // Standard Supabase implicit recovery redirect:
  // #access_token=...&refresh_token=...&type=recovery
  {
   ...
   await page.route('**/*',async route=>{
    ...
    if(u.hostname==='bxjvmjdppmqgbxfcowpf.supabase.co'){
      ...
      calls.push({path:u.pathname,method:req.method(),body});
      let response={};
      if(u.pathname.endsWith('/user'))response={id:'11111111-1111-4111-8111-111111111111',email:'test@example.com',aud:'authenticated'};
      return route.fulfill({status:200,contentType:'application/json',headers:{'access-control-allow-origin':'*','access-control-expose-headers':'x-supabase-api-version','x-supabase-api-version':'2024-01-01'},body:JSON.stringify(response)});
    }
    if(u.hostname!=='yamachat.test')throw Error('Unexpected external request: '+u.origin);
    const file=path.join(root,decodeURIComponent(u.pathname));
    return fs.existsSync(file)?route.fulfill({path:file}):route.fulfill({status:404,body:''});
   ...
   assert.equal(calls.filter(c=>c.path.endsWith('/verify')).length,0,'Implicit recovery must not call verifyOtp again');
   await page.locator('#newPassword').fill('ExamplePassword123!');
   await page.locator('#confirmNewPassword').fill('ExamplePassword123!');
   await page.locator('#passwordSubmit').click();
   await page.waitForFunction(()=>!document.getElementById('passwordSubmit').disabled);
   assert.equal(calls.filter(c=>c.path.endsWith('/user')&&c.method==='PUT').length,1);
   assert.match(await page.locator('#recoveryStatus').innerText(),/Heslo bylo změněno/);
   assert(await page.locator('#passwordForm').isHidden());
   assert.equal(await page.evaluate(()=>localStorage.getItem('existing-login')),'do-not-touch');
   assert.deepEqual(await page.evaluate(()=>Object.keys(localStorage)),['existing-login']);
   assert.deepEqual(errors,[]);
   await page.close();
   console.log('PASS recovery: implicit-session');
  }

  // A partial/malformed implicit redirect must never show a usable password form.
  {
   const page=await browser.newPage({viewport:{width:390,height:844}});
   await page.route('**/*',async route=>{
    const u=new URL(route.request().url());
    if(u.hostname==='bxjvmjdppmqgbxfcowpf.supabase.co')return route.fulfill({status:500,body:'{}'});
    if(u.hostname!=='yamachat.test')throw Error('Unexpected external request: '+u.origin);
    const file=path.join(root,decodeURIComponent(u.pathname));
    return fs.existsSync(file)?route.fulfill({path:file}):route.fulfill({status:404,body:''});
   });
   await page.goto('https://yamachat.test/reset-password.html#access_token='+encodeURIComponent(jwt())+'&type=recovery');
   await page.waitForTimeout(100);
   assert.equal(new URL(page.url()).hash,'');
   assert(await page.locator('#passwordForm').isHidden());
   assert(await page.locator('#retryLink').isVisible());
   await page.close();
   console.log('PASS recovery: malformed-implicit');
  }
