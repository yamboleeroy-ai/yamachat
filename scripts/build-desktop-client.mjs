...
const out=path.join(root,'build/desktop-profile-actions');fs.mkdirSync(out,{recursive:true});
fs.writeFileSync(path.join(out,'desktop-client.html'),withProfileActions(fs.readFileSync(path.join(ref,'desktop-client.html'),'utf8')));
console.log('Generated desktop client: build/desktop-profile-actions/desktop-client.html (requires the existing complete desktop runtime to package).');
