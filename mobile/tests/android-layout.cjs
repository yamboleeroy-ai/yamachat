...
if(output)fs.writeFileSync(path.join(output,'layout-results.json'),JSON.stringify(results,null,2));await browser.close();console.log('PASS '+results.length+' portrait layout cases, no duplicate Android navigation inset, idle/active voice');process.exit(0)
