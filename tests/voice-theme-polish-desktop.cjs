...
async function snap(page,{theme,server}){
 return page.evaluate(async({theme,server})=>{
  const root=document.documentElement,voice=document.getElementById('voiceControls'),panel=document.getElementById('voiceConnectionPanel'),messages=document.getElementById('messages'),composer=document.querySelector('.composer'),messageInput=document.getElementById('messageInput'),nav=document.getElementById('ycGlobalNav');
  ...
  messageInput?.focus();await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
  const focusedComposerBorder=composer?getComputedStyle(composer).borderColor:'',focusedComposerShadow=composer?getComputedStyle(composer).boxShadow:'';
  messageInput?.blur();
  return {
   ...
   composerBorder:cs?.borderColor||'',focusedComposerBorder,focusedComposerShadow,composerW:cr?.width||0,composerH:cr?.height||0,
...
(async()=>{
 ...
 try{
  for(const [width,height] of [[1440,900],[1100,650],[900,480]]){
   ...
   assert.notEqual(orangeB.composerBorder,purple.composerBorder,'Theme color must change actual message composer outline');
   assert.notEqual(orangeB.focusedComposerBorder,purple.focusedComposerBorder,'Theme color must change focused composer outline');
   assert.notEqual(orangeB.focusedComposerShadow,purple.focusedComposerShadow,'Theme color must change focused composer glow');
  ...
  console.log('PASS desktop theme polish: voice, actual chat composer, scrollbars and server settings follow the personal Yamachat theme; geometry remains stable.');
